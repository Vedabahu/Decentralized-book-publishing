      ...
      try {
        if (typeof window !== "undefined" && window.ethereum) {
          ...
          if (accounts.length > 0) {
            ...
            const userContract = getUserAuthContract(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (bookId !== null) {
        ...
        setStatus(
          "Transaction success, but failed to find BookCreated event in logs.",
        );
      }
    } catch (err: any) {
      console.error(err);
    ...
    return (
      <div className="max-w-xl mx-auto mt-10 p-6 bg-card text-card-foreground border border-border shadow-md rounded-md transition-colors duration-300">
        ...
        <p className="text-muted-foreground mb-6 font-medium">
          To create and sell books, you must register your Author Profile on the
          blockchain.
        </p>
        ...
        <form onSubmit={handleRegister} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col">
            <label className="mb-1 text-sm text-foreground font-semibold">
            ...
            <span className="text-xs text-muted-foreground mt-1">
              This will be securely uploaded and pinned to your profile via
              IPFS.
            </span>
          </div>

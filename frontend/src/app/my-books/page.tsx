...
export default function MyBooks() {
      ...
      try {
        if (typeof window !== "undefined" && window.ethereum) {
          ...
          try {
            ...
            console.error(
              "Failed to fetch user tokens from contract:",
              contractErr,
            );
  ...
  return (
    <main className="p-8 max-w-7xl mx-auto">
      <h1 className="text-3xl font-bold font-heading mb-8 text-foreground">
        My Library
      </h1>

      {books.length === 0 ? (
        <p className="text-muted-foreground text-lg">
          You do not own any books yet. Visit the Marketplace to buy some!
        </p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {books.map((book) => {
            ...
            return (
              <div
                key={book.tokenId}
                className="border border-border rounded-xl shadow-lg p-4 flex flex-col bg-card overflow-hidden transition-all hover:shadow-2xl hover:-translate-y-1"
              >
